...
import { CheckCheck, ChevronRight, GitCommitHorizontal, Loader2, TriangleAlert, X } from '@lucide/vue'
  ...
  <aside class="flex h-full flex-col border-l border-[var(--color-border)] bg-[var(--color-panel)]">
    <template v-if="viewingCommit">
      <div class="flex min-h-0 flex-1 flex-col">
        ...
        >
          <GitCommitHorizontal :size="13" class="shrink-0 text-[var(--color-accent)]" />
          <span class="min-w-0 flex-1 truncate normal-case">{{ selectedCommit?.subject ?? 'Commit' }}</span>
          <span class="font-mono text-[10px] text-[var(--color-fg-subtle)]">{{ repo.commitFiles.length }}</span>
        </header>
